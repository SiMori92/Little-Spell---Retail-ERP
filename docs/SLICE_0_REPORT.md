...
| 5 | `.env` and `state/` absent from the GitHub repo — checked | **DONE for tracked paths; current repo is PUBLIC by founder decision** | **Superseded finding:** on 2026-09-21, the GitHub API check against `SChiu-project/Little-Spell---Retail-ERP` found no `.env` or `state/` and reported `"isPrivate": true`. That private repository is no longer the deployment source. **Current check, 2026-09-23:** the GitHub API reports `SiMori92/Little-Spell---Retail-ERP` as `visibility: public`; its complete `main` tree has no `.env`, `state/`, `inbox/`, customer-data or tabular-data path. The sole `.env*` path is `.env.example`. Public visibility is intentional per the founder's 2026-09-23 decision. |
